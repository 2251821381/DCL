...
class DeepCrossNetworkModel(torch.nn.Module):
    ...
    def __init__(self, field_dims, embed_dim, num_layers, mlp_dims, dropout):
        ...
        self.linear = torch.nn.Linear(mlp_dims[-1] + self.embed_output_dim, 128)
...
class MvFS_DCN(nn.Module):
    def __init__(self, field_dims, embed_dim, num_selections=4):
        super().__init__()
        self.num = len(field_dims)
        self.embed_dim = embed_dim
        # self.emb = EMB(field_dims[:self.num], self.embed_dim)
        self.dcn = DeepCrossNetworkModel(field_dims, embed_dim=128, num_layers=1, mlp_dims=[16, 8], dropout=0.2)
